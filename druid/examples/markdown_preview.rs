...
fn rebuild_rendered_text(text: &str) -> RichText {
    ...
    for event in parser {
        match event {
            ...
            ParseEvent::Code(txt) => {
                ...
                attrs.push((range, Attribute::font_family(FontFamily::MONOSPACE)));
                current_pos += txt.len();
            }
            ParseEvent::Html(txt) => {
                buffer.push_str(&txt);
                let range = current_pos..current_pos + txt.len();
                attrs.push((range.clone(), Attribute::font_family(FontFamily::MONOSPACE)));
                attrs.push((range, Attribute::text_color(BLOCKQUOTE_COLOR)));
                current_pos += txt.len();
            }
    ...
    let mut text = RichText::new(buffer.into());
    for (range, attr) in attrs {
        text.add_attribute(range, attr);
    }
    text
}
...
fn add_attribute_for_tag(
    tag: &Tag,
    range: Range<usize>,
    attrs: &mut Vec<(Range<usize>, Attribute)>,
) {
    match tag {
        Tag::Heading(lvl) => {
            ...
            attrs.push((range.clone(), Attribute::size(font_size)));
            attrs.push((range, Attribute::weight(FontWeight::BOLD)));
        }
        Tag::BlockQuote => {
            attrs.push((range.clone(), Attribute::style(FontStyle::Italic)));
            attrs.push((range, Attribute::text_color(BLOCKQUOTE_COLOR)));
        }
        Tag::CodeBlock(_) => {
            attrs.push((range, Attribute::font_family(FontFamily::MONOSPACE)));
        }
        Tag::Emphasis => attrs.push((range, Attribute::style(FontStyle::Italic))),
        Tag::Strong => attrs.push((range, Attribute::weight(FontWeight::BOLD))),
        Tag::Link(..) => {
            attrs.push((range.clone(), Attribute::underline(true)));
            attrs.push((range, Attribute::text_color(LINK_COLOR)));
        }
